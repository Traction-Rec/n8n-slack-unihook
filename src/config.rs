...
pub struct Config {
    ...
    /// Optional shared secret for verifying inbound Jira webhook signatures.
    /// When set, the `X-Hub-Signature` header on incoming requests to
    /// `/jira/events` is verified using HMAC-SHA256 (Atlassian signing standard).
    /// When unset, inbound verification is skipped (existing behavior).
    #[serde(default)]
