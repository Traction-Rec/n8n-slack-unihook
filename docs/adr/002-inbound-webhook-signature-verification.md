...
External webhook providers (e.g. GitHub) support signing outbound payloads with
a shared secret so that receivers can verify authenticity. Without verification,
any party that knows the Unihook endpoint URL can inject fake events that will
be routed to n8n workflows.
...
`/github/events` endpoint actually originate from the expected provider.
...
### Why Jira is not included

n8n's Jira Trigger node does not use body-level HMAC verification for incoming
webhooks. Instead, it optionally validates a query parameter via the
`authenticateWebhook` / `httpQueryAuth` credential feature. Unihook supports
this transparently by forwarding query parameters from the inbound request URL
to the n8n webhook URL — no shared secret or HMAC verification is needed on the
Unihook side. See the README's
[Query Parameter Forwarding](../../README.md#query-parameter-forwarding-jira-authenticatewebhook)
section for details.
...
A `verify_hmac_sha256` function in `src/crypto.rs` handles parsing,
computation, and constant-time comparison.
...
- Operators must configure the same shared secret in both the external
  provider (GitHub webhook settings) and in Unihook's environment.
